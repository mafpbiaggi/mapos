...
class Os_model extends CI_Model
{
    ...
    public function autoCompleteProduto($q)
    {
        ...
        if ($query->num_rows > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label' => $row['descricao'] . ' | Preço: R$ ' . $row['precoVenda'] . ' | Estoque: ' . $row['estoque'], 'estoque' => $row['estoque'], 'id' => $row['idProdutos'], 'preco' => $row['precoVenda'], 'descricao' => $row['descricao']);
            }
    ...
    public function autoCompleteServico($q)
    {
        ...
        if ($query->num_rows > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = array('label' => $row['nome'] . ' | Preço: R$ ' . $row['preco'], 'id' => $row['idServicos'], 'preco' => $row['preco'], 'descricao' => $row['descricao']);
            }
            echo json_encode($row_set);
        }
    }
